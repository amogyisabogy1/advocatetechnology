...
function Terrain({ params }: { params: SceneParams }) {
  ...
  useFrame(({ clock }) => {
    if (meshRef.current && (params.turbulence ?? 0) > 0.5)
      meshRef.current.rotation.y = Math.sin(clock.elapsedTime * 0.15 * params.turbulence) * 0.03;
  });

  return (
    <mesh ref={meshRef} geometry={geo} receiveShadow>
      <meshStandardMaterial color={col} roughness={0.75} metalness={0.15} emissive={col} emissiveIntensity={0.08} />
    </mesh>
  );
}
  ...
  return (
    <mesh ref={meshRef} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.5, 0]}>
      ...
      <meshStandardMaterial
        color={col} roughness={0.05} metalness={0.85}
        transparent opacity={(params.waterOpacity ?? 0.85) * params.water}
...
function Forest({ params }: { params: SceneParams }) {
  const trees = useMemo(() => {
    ...
    const count = Math.floor(params.trees * 65);
    return Array.from({ length: count }, (_, i) => {
      const angle = (i / count) * Math.PI * 2 + rand(-0.4, 0.4);
      const r = rand(6, 30);
      return { x: Math.cos(angle) * r + rand(-3, 3), z: Math.sin(angle) * r + rand(-3, 3), s: rand(0.5, 1.7), type: Math.floor(rand(0, 3)) };
    });
  }, [params.trees]);

  const col = useMemo(() => hex(params.treeColor || "#22aa44"), [params.treeColor]);
  const trunkCol = useMemo(() => new THREE.Color("#6b3a1f"), []);
  // Lighter variant for emissive tip
  const tipCol = useMemo(() => col.clone().multiplyScalar(1.5), [col]);
        ...
        <group key={i} position={[t.x, 0, t.z]} scale={t.s}>
          {t.type === 0 && <>
            <mesh position={[0, 1.8, 0]} castShadow>
              ...
              <meshStandardMaterial color={col} roughness={0.85} emissive={tipCol} emissiveIntensity={0.15} />
            </mesh>
            <mesh position={[0, 0.9, 0]} castShadow>
              <coneGeometry args={[1.1, 2.5, 7]} />
              <meshStandardMaterial color={col} roughness={0.85} emissive={tipCol} emissiveIntensity={0.1} />
            </mesh>
          </>}
          {t.type === 1 && <mesh position={[0, 2, 0]} castShadow>
            <sphereGeometry args={[1.2, 10, 8]} />
            <meshStandardMaterial color={col} roughness={0.8} emissive={tipCol} emissiveIntensity={0.12} />
          </mesh>}
          {t.type === 2 && <mesh position={[0, 2.5, 0]} castShadow>
            <coneGeometry args={[0.6, 5.5, 5]} />
            <meshStandardMaterial color={col} roughness={0.9} emissive={tipCol} emissiveIntensity={0.1} />
          </mesh>}
            ...
            {b.lit && (
              <>
                <mesh position={[b.x, b.h * 0.7, b.z]}>
                  ...
                  <meshStandardMaterial color={winCol} emissive={winCol} emissiveIntensity={3} />
                </mesh>
...
function Ruins({ params }: { params: SceneParams }) {
  const cols = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, i) => {
      const angle = (i / count) * Math.PI * 2;
      const r = rand(5, 14);
      return { x: Math.cos(angle) * r + rand(-2, 2), z: Math.sin(angle) * r + rand(-2, 2), h: rand(3, 10), broken: Math.random() > 0.5, lean: rand(-0.05, 0.05) };
    });
  }, [params.columns]);

  const col = useMemo(() => new THREE.Color("#d4c5a0"), []);
  const glowCol = useMemo(() => new THREE.Color("#e8dfc0"), []);

  return (
    <>
      {cols.map((c, i) => (
        <group key={i} position={[c.x, 0, c.z]} rotation={[c.lean, 0, c.lean * 0.5]}>
          <mesh position={[0, (c.broken ? c.h * 0.5 : c.h) / 2, 0]} castShadow>
            <cylinderGeometry args={[0.3, 0.38, c.broken ? c.h * 0.55 : c.h, 12]} />
            <meshStandardMaterial color={col} roughness={0.85} emissive={glowCol} emissiveIntensity={0.1} />
          </mesh>
          {!c.broken && (
      ...
      {fires.map((f, i) => (
        <group key={i} position={[f.x, 0, f.z]}>
          <mesh ref={el => { refs.current[i] = el; }} position={[0, f.h * 0.5, 0]}>
            ...
            <meshStandardMaterial color="#ff3300" emissive="#ff4400" emissiveIntensity={4} transparent opacity={0.9} />
          </mesh>
          <mesh position={[0, f.h * 0.28, 0]}>
            <coneGeometry args={[f.r * 0.5, f.h * 0.6, 7]} />
            <meshStandardMaterial color="#ffdd00" emissive="#ffcc00" emissiveIntensity={5} transparent opacity={0.75} />
          </mesh>
          <pointLight position={[0, f.h * 0.5, 0]} color="#ff6600" intensity={params.fire * 16} distance={14} decay={2} />
          <pointLight position={[0, 0.2, 0]} color="#ff3300" intensity={params.fire * 6} distance={6} decay={2} />
        </group>
      ))}
    </>
  );
...
function Spheres({ params }: { params: SceneParams }) {
  const items = useMemo(() => {
    ...
    return Array.from({ length: count }, () => ({
      ...
      r: rand(0.3, 2.0) * params.spheres,
      phase: rand(0, Math.PI * 2), speed: rand(0.2, 0.8),
      hue: Math.random(),
    }));
  }, [params.spheres]);

  const groupRef = useRef<THREE.Group>(null);
  const baseCol = useMemo(() => hex(params.sphereColor || "#8866ff"), [params.sphereColor]);

  useFrame(({ clock }) => {
    if (!groupRef.current) return;
    groupRef.current.children.forEach((child, i) => {
      if (i < items.length)
        child.position.y = items[i].y + Math.sin(clock.elapsedTime * items[i].speed + items[i].phase) * 1.4;
    });
  });
        ...
        return (
          <group key={i} position={[s.x, s.y, s.z]}>
            <mesh>
              ...
              <meshStandardMaterial color={col} emissive={col} emissiveIntensity={1.2} roughness={0.05} metalness={0.95} transparent opacity={0.92} />
            </mesh>
            <pointLight color={col} intensity={2} distance={s.r * 8} decay={2} />
          </group>
        );
      })}
    </group>
...
function Rings({ params }: { params: SceneParams }) {
  const items = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      r: rand(1.5, 6), rx: rand(0, Math.PI), ry: (i / count) * Math.PI * 2,
      ...
      {items.map((ring, i) => {
        ...
        return (
          <mesh key={i} position={[ring.x, ring.y, ring.z]}>
            ...
            <meshStandardMaterial color={col} emissive={col} emissiveIntensity={2.5} transparent opacity={0.8} />
          </mesh>
        );
      })}
    </group>
...
function ParticleSystem({ params }: { params: SceneParams }) {
  ...
  const count = Math.min(8000, Math.floor((params.particleCount ?? 2000) * Math.max(pDensity, 0.4)));
  ...
  return (
    <points>
      ...
      <pointsMaterial color={pColor} size={pSize} sizeAttenuation transparent opacity={0.88} depthWrite={false} />
    </points>
  );
}

// ─── Aurora ───────────────────────────────────────────────────────────────────
// Ribbon-like glowing bands for cosmos / dream scenes

function Aurora({ params }: { params: SceneParams }) {
  if ((params.stars ?? 0) < 0.5 && (params.spheres ?? 0) < 0.4) return null;
  const meshRef = useRef<THREE.Mesh>(null);
  const col1 = useMemo(() => new THREE.Color().setHSL(Math.random() * 0.2 + 0.45, 1, 0.6), []);
  const col2 = useMemo(() => new THREE.Color().setHSL(Math.random() * 0.2 + 0.65, 1, 0.55), []);

  useFrame(({ clock }) => {
    if (!meshRef.current) return;
    meshRef.current.position.y = 12 + Math.sin(clock.elapsedTime * 0.3) * 2;
    meshRef.current.rotation.z = Math.sin(clock.elapsedTime * 0.15) * 0.08;
  });

  const geo = useMemo(() => {
    const g = new THREE.PlaneGeometry(80, 8, 40, 4);
    const pos = g.attributes.position as THREE.BufferAttribute;
    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i);
      pos.setZ(i, Math.sin(x * 0.15) * 3);
    }
    g.computeVertexNormals();
    return g;
  }, []);
  ...
  return (
    <mesh ref={meshRef} geometry={geo} position={[0, 14, -20]} rotation={[0.2, 0, 0]}>
      <meshStandardMaterial
        color={col1} emissive={col2} emissiveIntensity={1.8}
...
function GroundGlow({ params }: { params: SceneParams }) {
  ...
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.05, 0]}>
      ...
      <meshStandardMaterial color={col} emissive={col} emissiveIntensity={0.25} roughness={1} />
    </mesh>
  );
}
...
function Lighting({ params }: { params: SceneParams }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-sunX * 0.5, sunY * 0.3, -15]} intensity={intensity * 0.25} color={lightCol} />
    </>
  );
}
...
function PostFX({ params }: { params: SceneParams }) {
  const hasFire = (params.fire ?? 0) > 0.3;
  const hasStars = (params.stars ?? 0) > 0.5;
  const hasRings = (params.rings ?? 0) > 0.2;
  const hasSpheres = (params.spheres ?? 0) > 0.2;
  const bloomStrength = hasFire ? 1.4 : hasRings || hasSpheres ? 1.1 : hasStars ? 0.8 : 0.6;
...
function SceneContent({ params }: { params: SceneParams }) {
  ...
  return (
    <>
      <fog attach="fog" args={[fogCol, 14, 85]} />
      ...
      <Rings params={params} />
      <Aurora params={params} />
      <ParticleSystem params={params} />
      {(params.stars ?? 0) > 0.2 && (
        <Stars
          radius={90} depth={60}
          count={Math.floor(params.stars * 7000)}
          factor={5} saturation={0.8} fade
          speed={(params.turbulence ?? 0) * 0.5 + 0.08}
        />
      )}
...
function GradientFallback({ params }: { params: SceneParams }) {
  return (
    <div className="absolute inset-0" style={{
      background: `radial-gradient(ellipse at 40% 60%, ${params.groundColor || "#2a1060"} 0%, ${params.skyColor || "#060818"} 100%)`,
    }} />
  );
}
...
export function SceneCanvas({ params, isTransitioning }: SceneCanvasProps) {
  ...
  if (webglOk === false) {
    return <div className="absolute inset-0" style={style}><GradientFallback params={params} /></div>;
  }
